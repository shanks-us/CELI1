...
ovlcmd(
    ...
    async (ms_org, ovl, cmd_options) => {
        try {
             const rawUrl = 'https://files.catbox.moe/rb0a4f.jpg';
            ...
            const menu = `🌐 Bienvenue sur *CELIBATAIRE*, votre bot si tu es pas celibataire faut te déconnecter à notre robot.🔍 Tapez *${config.PREFIXE}menu* pour voir toutes les commandes disponibles.\n> ©2025 CELIBATAIRE By *𝙺𝙰𝚆𝙰𝙺𝙸²²7*`;
...
ovlcmd(
  ...
  async (ms_org, ovl, cmd_options) => {
    ...
    try {
      const rawUrl = 'https://raw.githubusercontent.com/CELIBATAIRE-devs/CELIBATAIRE-THEME/refs/heads/main/themes.json';
      ...
      if (sousCmd === "list") {
        ...
        return ovl.sendMessage(ms_org, {
          image: { url: 'https://files.catbox.moe/rb0a4f.jpg' },
          caption: msg
        }, { quoted: ms });
      }
...
> ©2025 CELIBATAIRE By *kawaki227*`;
            ...
            const rawUrl = 'https://files.catbox.moe/6xlk10.jpg';
            const { data: themes } = await axios.get(rawUrl);
            const selectedTheme = themes.find(t => t.id == config.THEME);
            if (!selectedTheme) throw new Error("Thème introuvable dans le fichier JSON");
            const lien = selectedTheme.theme[Math.floor(Math.random() * selectedTheme.theme.length)];
...
╰──────────────────╯\n\n`;
            ...
            const rawUrl = 'https://raw.githubusercontent.com/kawaki227-devs/CELIBATAIRE-THEME/refs/heads/main/themes.json';
            const { data: themes } = await axios.get(rawUrl);
            const selectedTheme = themes.find(t => t.id == config.THEME);
            if (!selectedTheme) throw new Error("Thème introuvable dans le fichier JSON");
            const lien = selectedTheme.theme[Math.floor(Math.random() * selectedTheme.theme.length)];
...
ovlcmd(
    ...
    async (ms_org, ovl, cmd_options ) => {
        ...
        await ovl.sendMessage(ms_org, { text: "*CELIBATAIRE Ping...*" }, { quoted: cmd_options.ms });
        const end = Date.now();
