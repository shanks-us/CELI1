...
async function sendMedia(ms_org, ovl, url, format, type, ms) {
  try {
    ...
    const message = {
      ...
      caption: "```Powered By 𝙲𝙴𝙻𝙸𝙱𝙰𝚃𝙰𝙸𝚁𝙴```"
    };
...
ovlcmd(
    ...
    async (ms_org, ovl, cmd_options) => {
        ...
        try {
            ...
            const caption = `╭─── 〔 𝙲𝙴𝙻𝙸𝙱𝙰𝚃𝙰𝙸𝚁𝙴 VIDEO 〕 ──⬣\n⬡ Titre: ${videoInfo.title}\n⬡ URL: ${videoInfo.url}\n⬡ Vues: ${videoInfo.views}\n⬡ Durée: ${videoInfo.duration}\n╰───────────────────⬣`;
...
ovlcmd(
  ...
  async (ms_org, ovl, cmd_options) => {
    ...
    try {
      ...
      return ovl.sendMessage(ms_org, { video: videoBuffer, caption: `\`\`\`Powered By 𝙲𝙴𝙻𝙸𝙱𝙰𝚃𝙰𝙸𝚁𝙴\`\`\`` }, { quoted: ms });
...
ovlcmd(
  ...
  async (ms_org, ovl, cmd_options) => {
    ...
    try {
      ...
      return ovl.sendMessage(ms_org, { video: Buffer.from(video.data), caption: `\`\`\`Powered By 𝙲𝙴𝙻𝙸𝙱𝙰𝚃𝙰𝙸𝚁𝙴\`\`\`` }, { quoted: ms });
