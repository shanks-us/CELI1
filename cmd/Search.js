...
ovlcmd(
    ...
    async (ms_org, ovl, cmd_options) => {
      const { arg, ms } = cmd_options;
        ...
        if (!searchTerm) {
            return ovl.sendMessage(ms_org, { text: "Veuillez fournir un terme de recherche, par exemple : img CELIBATAIRE" },  { quoted: ms });
        }
            ...
            for (const image of images) {
                try {
                    await ovl.sendMessage(ms_org, {
                        image: { url: image.url },
                        caption: `\`\`\`Powered By CELIBATAIRE\`\`\``
                    }, { quoted: ms });
                } catch (err) {
...
ovlcmd(
    ...
    async (ms_org, ovl, cmd_options) => {
        ...
        try {
            ...
            const mess = `🎸CELIBATAIRE LYRICS FINDER🥁\n\n🎼PAROLES =>\n\n${lyrics}`;
            if (!lyrics) {
...
ovlcmd(
  ...
  async (ms_org, ovl, cmd_options) => {
    ...
    try {
      ...
      let messageText = "*🔍CELIBATAIRE APK-SEARCH:*\n\n";
